		...
		const subscription = supabase
			...
			.on(
				...
				(payload) => {
					...
					setReports((currentReports) => {
						const newReports = currentReports.map((report) => (report.id === payload.new.id ? payload.new : report))
						setCachedReports(newReports)
						return newReports
					})
				}
			...
			<div style={{ maxWidth: "1200px", margin: "0 auto" }} className={isDarkMode ? "dark-mode" : ""}>
				...
				<div id='content' style={{ paddingTop: "20px", paddingBottom: "900px" }}>
					<Routes>
						...
						<Route path='/past-reports' element={<PastReports reports={sortedReports} loading={loadingReports} />} />
						<Route path='/hours-table' element={<HoursTable reports={reports} loading={loadingReports} />} />
...
const desktopLinkStyle = (isActive: boolean, isDarkMode: boolean): React.CSSProperties => ({
	textDecoration: "none",
	color: isActive ? (isDarkMode ? "#0A84FF" : "#007AFF") : isDarkMode ? "#98989F" : "#555",
	fontSize: "16px",
...
const mobileLinkStyle = (isActive: boolean, isDarkMode: boolean): React.CSSProperties => ({
	...
	alignItems: "center",
	textDecoration: "none",
	color: isActive ? (isDarkMode ? "#0A84FF" : "#007AFF") : isDarkMode ? "#98989F" : "#8E8E93",
	fontSize: "12px",
})
